...
def register_nodes(graph: StateGraph, llm: ChatOpenAI, icp_tool: list, tools: list, tools_2: list):
    graph.add_node("chatbot", create_orchestrator_node(llm, icp_tool))
    graph.add_node("tools", ToolNode(tools=icp_tool))
    graph.add_node("search_tool", ToolNode(tools=tools))
    graph.add_node("search_tool_enricher", ToolNode(tools=tools_2))
    graph.add_node("lead_finder", create_lead_finder_node(llm, tools))
    graph.add_node("screener", lead_screener_node)
    graph.add_node("enricher", create_enrichment_node(llm, tools_2))
